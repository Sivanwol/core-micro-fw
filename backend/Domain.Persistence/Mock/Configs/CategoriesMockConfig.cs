...
public sealed class CategoriesMockConfig : AutoFaker<Categories> {
    public CategoriesMockConfig() {
        Randomizer.Seed = new Random(8675309);
        RuleFor(fake => fake.Id, fake => fake.Random.Number(1, 1000));
        RuleFor(fake => fake.Name, fake => $"Category {fake.Random.Number(1, 1000)}");
